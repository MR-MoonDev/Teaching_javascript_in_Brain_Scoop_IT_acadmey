...
function displayMessageWithoutParameter() {
  document.getElementById("fun").innerHTML +=
    "ONE FUNCTION WITHOUT PARAMETER</br><br>";
}
...
function displaySumWithoutReturn(num1, num2) {
  document.getElementById("fun").innerHTML += `without return  ${
    num1 + num2
  }</br><br>`;
}
...
document.getElementById(
  "fun2"
).innerHTML += `add 2 in with return ${sumResultWithExtra}</br><br>`;
...
// arrow_fun_with_parameter()

let a = arrow_fun_with_parameter(2, 3);
let arrow_fun_with_parameter_holder = a;
document.getElementById("fun2").innerHTML += `Arrow function with parameter Ans is =  ${arrow_fun_with_parameter_holder}</br><br>`;
